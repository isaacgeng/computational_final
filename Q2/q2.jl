...
let
    ...
    final_v = zeros(nk,nz)
    for iz =1:2
        c0[:,iz] = zgrid[iz].*(kgrid.^α).*(N.^(1-α)) + (1-δ).*kgrid .- 0.2;
    end
...
p = Gadfly.plot(x=kgrid, y=final_v[:,2], Geom.line)
img = SVG("yaoming.svg", 6inch, 4inch)
draw(img, p)
p2 = Gadfly.plot(x=kgrid,y=c_argmax[:,2],Geom.line)
img2 = SVG("policy_c_yaoming.svg",6inch,4inch)
draw(img2,p2)

p3 = Gadfly.plot(x=kgrid,y=l_argmax[:,2],Geom.line)
img3 = SVG("policy_l_yaoming.svg",6inch,4inch)
draw(img3,p3)
end
